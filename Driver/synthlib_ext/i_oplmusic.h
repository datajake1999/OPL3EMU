...
private:
    ...
    opl_voice_t voices[OPL_NUM_VOICES * 2 * 8];
    opl_voice_t *voice_free_list[OPL_NUM_VOICES * 2 * 8];
    opl_voice_t *voice_alloced_list[OPL_NUM_VOICES * 2 * 8];
    ...
    void OPL_WriteRegister(unsigned int chip, unsigned int reg, unsigned char data);
    void OPL_InitRegisters(unsigned int chip);
    bool LoadInstrumentTable(void);
    void ReleaseVoice(unsigned int id);
    void LoadOperatorData(unsigned int chip, int slot, genmidi_op_t *data, bool max_level, unsigned int *volume);
    ...
    void SetVoicePanMIDI(opl_voice_t *voice, unsigned int pan);
