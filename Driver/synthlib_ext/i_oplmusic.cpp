...
void DoomOPL::LoadOperatorData(unsigned int chip, int slot, genmidi_op_t *data,
...
void DoomOPL::SetVoiceVolume(opl_voice_t *voice, unsigned int volume)
{
    ...
    if (car_volume != (voice->car_volume & 0x3f))
    {
        ...
        OPL_WriteRegister(voice->chip, (OPL_REGS_LEVEL + voice->op2) | voice->array, voice->car_volume);
        ...
        if ((opl_voice->feedback & 0x01) != 0 && opl_voice->modulator.level != 0x3f)
        {
            ...
            if (mod_volume != voice->mod_volume)
            {
                ...
                OPL_WriteRegister(voice->chip, (OPL_REGS_LEVEL + voice->op1) | voice->array,
                                  voice->mod_volume);
            }
        }
    }
...
void DoomOPL::SetVoicePanMIDI(opl_voice_t *voice, unsigned int pan)
{
    genmidi_voice_t *opl_voice;

    OPL_WriteRegister(voice->chip, (OPL_REGS_PAN + voice->index) | voice->array, pan << 1);
}
                       ...
                       unsigned int volume)
{
    ...
    voice->freq = 0;
    SetVoicePanMIDI(voice, channel->pan_midi);
...
void DoomOPL::SetChannelPan(opl_channel_data_t *channel, unsigned int pan)
{
    ...
    for (i = 0; i < voice_alloced_num; i++)
    {
        if (voice_alloced_list[i]->channel == channel)
        {
            SetVoicePanMIDI(voice_alloced_list[i], pan);
        }
    }
}
...
void DoomOPL::PitchBendEvent(unsigned char channel_num, unsigned char bend)
{
    opl_channel_data_t *channel;
    unsigned int i;
    opl_voice_t *voice_updated_list[OPL_NUM_VOICES * 2 * 8];
    unsigned int voice_updated_num = 0;
    opl_voice_t *voice_not_updated_list[OPL_NUM_VOICES * 2 * 8];
...
int DoomOPL::midi_init(unsigned int rate)
{
	...
	if (env)
	{
        ...
        if (pos = strstr(env, "-chips"))
        {
            ...
            opl_new = 1;
            opl_voices = OPL_NUM_VOICES * 2 * num_chips;
        }
...
void DoomOPL::midi_generate(signed short *buffer, unsigned int length) {
    ...
    {
        opl[i]->fm_generate(buffer, length);
    }
}
